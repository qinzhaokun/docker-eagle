...
: ${EAGLE_HOST:? eagle server address is mandatory, fallback is a linked containers exposed 7070}
...
get-server-state() {
  curl -s -o /dev/null -w "%{http_code}" $AMBARISERVER_PORT_9099_TCP_ADDR:9099/eagle/index.html
}
